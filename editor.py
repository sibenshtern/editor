...
from graphical import (Controller, BlockFrame, PortItem, InstanceItem,
                       WireItem,
                       JunctionItem)
...
class Editor(QMainWindow):
    ...
    def _save_version(self, action_name: str):
        ...
        try:
            ...
            self.version_manager.save_state(action_name,
                                            save_to_file=self.current_file_path)
    ...
    def _restore_from_version(self):
        ...
        self._rebuild_object_model_from_graphical(self.current_file_path)
    ...
    def _rebuild_object_model_from_graphical(self, file_path: str,
        ...
        # Clear object model
        if open_file:
            self.netlist_project, _ = self.parser.load_netlist_from_file(
                file_path)
        else:
            self.netlist_project = NetlistProject("circuit_netlist")
            ...
            for block_id, block_frame in self.controller.blocks.items():
                ...
                for port_model in block_frame.model.ports:
                    self.netlist_project.add_pin_to_block(block_name,
                                                          port_model.name)
                ...
                for wire_model in block_frame.model.wires:
                    if wire_model.name:
                        ...
                        if net_name not in self.netlist_project.blocks[
                            block_name].nets:
                            self.netlist_project.add_net_to_block(block_name,
                                                                  net_name)
                        ...
                        for endpoint in [wire_model.start, wire_model.end]:
                            if endpoint and len(endpoint) >= 2:
                                ...
                                if not owner_id.startswith("j:") and pin_name:
                                    pin_ref = self._get_pin_ref(block_name,
                                                                owner_id,
                                                                pin_name)
                                    if pin_ref:
                                        self.netlist_project.blocks[
                                            block_name].nets[
                                            net_name].connect_pin(pin_ref)
    ...
    def _get_pin_ref(self, block_name: str, owner_id: str, pin_name: str):
        ...
        if owner_id.startswith("block:"):
            ...
            if block_frame and block_frame.model.name == block_name:
                return self.netlist_project.blocks[
                    block_name].interface_pins.get(pin_name)
        else:
            # Instance pin
            for inst_model in self.controller.blocks[
                self._current_block_id].model.instances:
                if inst_model.id == owner_id:
                    data_inst = self.netlist_project.blocks[
                        block_name].instances.get(inst_model.name)
                    if data_inst:
    ...
    def open_file(self):
        ...
        if file_path:
            ...
            if not os.path.exists(graphical_file):
                QMessageBox.critical(self, "Error",
                                     f"Graphical file not found: {graphical_file}")
                return
            if not os.path.exists(netlist_file):
                QMessageBox.critical(self, "Error",
                                     f"Netlist file not found: {netlist_file}")
                return

            try:
                # Create new version manager for this file (will load existing repository if available)
                self.version_manager = VersionManager(file_path,
                                                      self.controller)
                ...
                self._rebuild_object_model_from_graphical(netlist_file, True)
        ...
        if self.current_file_path is None:
            ...
            try:
                ...
                if self.version_manager.file_path != self.current_file_path:
                    self.version_manager.associate_with_file(
                        self.current_file_path)

                self.controller.save_scene(self.current_file_path)
                # Save version after saving file
                self._save_version(
                    f"Save file {os.path.basename(self.current_file_path)}")
                self.parser.save_netlist_to_file(
                    f"{self.current_file_path.split('.')[0]}.net",
                    self.netlist_project)
                QMessageBox.information(self, "Success",
    ...
    def save_file_as(self):
        ...
        if file_path:
            ...
            try:
                ...
                # Save version after saving file
                self._save_version(
                    f"Save file as {os.path.basename(file_path)}")
                self.parser.save_netlist_to_file(
                    f"{self.current_file_path.split('.')[0]}.net",
                    self.netlist_project)
    ...
    def add_block(self):
        ...
        name = self.__get_new_name("New block", "Block name:",
                                   f"Block{len(self.controller.blocks) + 1}")
        if not name:
            return

        if self._block_name_exists(name):
            QMessageBox.warning(self, "Error",
                                f"Block name '{name}' already exists.")
            return
        ...
        for bf_item in self.controller.blocks.values():
            if bf_item.model.name not in self.netlist_project.blocks:
                try:
                    ...
                    QMessageBox.warning(self, "Error",
                                        f"Failed to add block to model: {e}")
        self.refresh_objects_list()
        for i in range(self.ui.objects_list.count()):
    ...
    def delete_block(self):
        ...
        if not blocks:
            QMessageBox.information(self, "Delete Block",
    ...
    def copy_block(self):
        ...
        blocks = [it for it in selected if isinstance(it, BlockFrame)]
        if not blocks:
            QMessageBox.information(self, "Copy Block",
        ...
        if self._block_name_exists(new_name):
            QMessageBox.warning(self, "Error",
                                f"Block name '{new_name}' already exists.")
            return
    ...
    def rename_block(self):
        ...
        blocks = [it for it in selected if isinstance(it, BlockFrame)]
        if not blocks:
            QMessageBox.information(self, "Rename Block",
        ...
        if new_name:
            ...
            try:
                ...
                self.refresh_objects_list()
                self._save_version(
                    f"Rename block '{old_name}' to '{new_name}'")
            except Exception as e:
                QMessageBox.warning(self, "Error",
                                    f"Failed to rename block: {e}")
        ...
        def handler(ev):
            ...
            if parent_frame.mapRectToScene(parent_frame.rect()).contains(pos):
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Error",
                                        f"Failed to add instance: {e}")
                    self.view.viewport().removeEventFilter(filter_obj)
                    return True
                if not self._sync_instance_added(parent_name,
                                                 inst_item.model.name,
                                                 child_frame.model.name):
                    self.controller.delete_instance(inst_item)
                    self.view.viewport().removeEventFilter(filter_obj)
                    return True
                self._save_version(
                    f"Add instance '{inst_item.model.name}' to '{parent_name}'")
                self.refresh_objects_list()
                self.view.viewport().removeEventFilter(filter_obj)
                return True
            return False
    ...
    def delete_instance(self):
        ...
        for inst in instances:
            ...
            if isinstance(parent_block, BlockFrame):
                ...
                if not self._sync_instance_removed(block_name,
                                                   inst.model.name):
                    continue
    ...
    def copy_instance(self):
        ...
        instances = [it for it in selected if isinstance(it, InstanceItem)]
        if not instances:
            QMessageBox.information(self, "Copy Instance",
                                    "Select an instance to copy.")
            return
        ...
        if not isinstance(parent_block, BlockFrame):
            QMessageBox.warning(self, "Error",
                                "Instance parent is not a BlockFrame.")
            return
        ...
        if self._instance_name_exists_in_block(parent_block, new_name):
            QMessageBox.warning(self, "Error",
                                f"Instance name '{new_name}' already exists in this block.")
            return
        ...
        try:
            ...
            if not self._sync_instance_added(parent_block.model.name, new_name,
                                             inst.model.block_name):
                self.controller.delete_instance(new_inst_item)
                return
            self._save_version(
                f"Copy instance '{inst.model.name}' to '{new_name}'")
            self.refresh_objects_list()
        except Exception as e:
    ...
    def rename_instance(self):
        ...
        if new_name:
            ...
            try:
                ...
                if not self._sync_instance_renamed(block_name, old_name,
                                                   new_name):
                    return
                self._save_version(
                    f"Rename instance '{old_name}' to '{new_name}'")
                inst.model.name = new_name
                inst.title.setText(new_name)
            except Exception as e:
                QMessageBox.warning(self, "Error",
                                    f"Failed to rename instance: {e}")
                ...
                if block_frame:
                    ...
                    if block_name in self.netlist_project.blocks:
                        if not self._sync_pin_removed(block_name, pin_name):
                            continue
                self._save_version(
                    f"Delete pin '{pin_name}' from block '{block_name}'")
    ...
    def rename_pin(self):
        ...
        pins = [element for element in selected if
                isinstance(element, PortItem)]

        if not pins:
            QMessageBox.information(self, "Rename Pin",
                                    "Select a pin to rename.")
            return
        ...
        if not pin.owner_id.startswith("block:"):
            QMessageBox.information(self, "Rename Pin",
                                    "Only block-level pins can be renamed.")
            return

        new_name = self.__get_new_name("Rename Pin", "New name:",
                                       pin.model.name)
    ...
    def add_net(self):
        ...
        def handler(ev):
            ...
            if self.controller.temp_wire_start is None:
                ...
                block_frame = self.controller.blocks.get(
                ...
                if block_frame and isinstance(item, PortItem):
                    if self._is_pin_already_connected(item, block_frame):
                        QMessageBox.warning(None, "Error",
                                            "Cannot connect to a pin that is already connected to a net.")
                        self._deactivate_mode()
                        return True
                ...
                if block_frame and isinstance(item, PortItem):
                    if self._is_pin_already_connected(item, block_frame):
                        QMessageBox.warning(None, "Error",
                                            "Cannot connect to a pin that is already connected to a net.")
                        self.controller._restore_pin_color(
                            self.controller.temp_wire_start)
                ...
                if isinstance(new_wire, WireItem) and block_frame:
                    ...
                    net_name = getattr(wire_model, "name",
                                       None) or self._generate_net_name(
                        block_frame)
                    ...
                    start_type, start_id, start_name = get_type_and_id_and_name(
                        wire_model.start)
                    end_type, end_id, end_name = get_type_and_id_and_name(
                        wire_model.end)
                    ...
                    if start_type == 'j' and end_type in ('block', 'inst'):
                        ...
                        if end_type == 'block':
                            pin = self.netlist_project.blocks[
                                pin_block_name].interface_pins[end_name]
                        else:
                            name = functions[end_type](end_id).block_name
                            for instance in \
                            self.netlist_project.blocks_instances[name]:
                        ...
                        if end_type == 'block':
                            pin = self.netlist_project.blocks[
                                pin_block_name].interface_pins[start_name]
                        else:
                        ...
                        self.netlist_project.remove_net_from_block(block_name,
                                                                   end_wire.name)
                        ...
                        new_wire.model.name = None
                    elif start_type in ('block', 'inst') and end_type in (
                            'block', 'inst'):
                        ...
                        if start_type == 'block':
                            ...
                            for instance in \
                                ...
                                if instance.name == start_block_name:
                                    start_pin = instance.interface_pins[
                                        start_name]
                        ...
                        if wire_model and not getattr(wire_model, "name",
                                                      None):
                            wire_model.name = net_name
                        if hasattr(new_wire, "update_label"):
                            new_wire.update_label(net_name)
                        if not self._sync_net_added(block_name, net_name,
                                                    [start_pin, end_pin]):
                            self.controller.delete_wire(new_wire)
                            self._deactivate_mode()
                            return True
                    self._save_version(
                            f"Add net '{net_name}' in block '{block_name}'")
                    self._log_object_model()
                self._deactivate_mode()
                return True
    ...
    def _is_pin_already_connected(self, port_item: PortItem,
                                  block_frame: BlockFrame) -> bool:
    ...
    def delete_net(self):
        ...
        if not wires:
            QMessageBox.information(self, "Delete Wire",
                                    "Select a wire to delete.")
            return
        ...
        connected_wires = [
            ...
            for wire_id in
            self._find_connected_wires(wire.model.id, block_frame)
        ]
    ...
    def rename_net(self):
        ...
        wires = [it for it in selected if isinstance(it, WireItem)]
        if not wires:
            QMessageBox.information(self, "Rename Net",
                                    "Select a wire to rename.")
            return
        wire = wires[0]
        block_frame = self.controller.blocks.get(self._current_block_id)
        if not block_frame:
            QMessageBox.warning(self, "Error",
                                "No block context for selected wire.")
            return
        ...
        if not old_name and len(junctions) == 0:
            QMessageBox.warning(self, "Error",
                                "Selected wire has no name to rename.")
            return
        ...
        if self._net_name_exists(block_frame, new_name, exclude=old_name):
            QMessageBox.warning(self, "Error",
                                f"Net name '{new_name}' already exists in this block.")
            return
    ...
    def _find_connected_wires(self, wire_id: str,
                              block_frame: BlockFrame) -> set:
        ...
        while to_visit_wires:
            ...
            for wm in block_frame.model.wires:
                if wm.id == current_wire_id:
                    # Проверяем start
                    if wm.start and len(wm.start) > 0 and wm.start[
                        0].startswith("j:"):
                        jid = wm.start[0].split(":", 1)[1]
                        if jid not in visited_junctions:
                            connected_junctions.add(jid)
                    # Проверяем end
                    if wm.end and len(wm.end) > 0 and wm.end[0].startswith(
                            "j:"):
            ...
            for junction_id in connected_junctions:
                ...
                for wm in block_frame.model.wires:
                    ...
                    if wm.start and len(wm.start) > 0 and wm.start[
                        0] == f"j:{junction_id}":
                        is_connected = True
                    if wm.end and len(wm.end) > 0 and wm.end[
                        0] == f"j:{junction_id}":
                        is_connected = True
    ...
    def redo(self):
        ...
        if self.version_manager.redo():
            self._restore_from_version()
            QMessageBox.information(self, "Redo",
    ...
    def _instance_name_exists_in_block(self, block_frame: BlockFrame,
                                       name: str,
    ...
    def _sync_block_removed(self, block_name: str) -> bool:
        ...
        try:
            ...
            QMessageBox.warning(self, "Error",
                                f"Failed to delete block from model: {e}")
            return False
    ...
    def _sync_instance_added(self, parent_block: str, instance_name: str,
                             source_block: str) -> bool:
        if parent_block not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Parent block '{parent_block}' not found in model.")
            return False
        try:
            self.netlist_project.add_instance_to_block(parent_block,
                                                       instance_name,
                                                       source_block)
            self._log_object_model()
            return True
            ...
            QMessageBox.warning(self, "Error",
                                f"Failed to add instance to model: {e}")
            return False

    def _sync_instance_removed(self, parent_block: str,
        ...
        if parent_block not in self.netlist_project.blocks:
            ...
            return False
        try:
            self.netlist_project.remove_instance_from_block(parent_block,
                                                            instance_name)
            self._log_object_model()
            return True
        except Exception as e:
            QMessageBox.warning(self, "Error",
                                f"Failed to delete instance: {e}")
            return False

    def _sync_instance_renamed(self, parent_block: str, old_name: str,
                               new_name: str) -> bool:
        if parent_block not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Parent block '{parent_block}' not found in model.")
            return False
        try:
            self.netlist_project.rename_instance_in_block(parent_block,
                                                          old_name, new_name)
            self._log_object_model()
            return True
        except Exception as e:
    ...
    def _sync_pin_added(self, block_name: str, pin_name: str) -> bool:
        if block_name not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Block '{block_name}' not found in model.")
            return False
        try:
            ...
            QMessageBox.warning(self, "Error",
                                f"Failed to add pin to model: {e}")
            return False
    ...
    def _sync_pin_renamed(self, block_name: str, old_name: str,
                          new_name: str) -> bool:
        if block_name not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Block '{block_name}' not found in model.")
            return False
        try:
            self.netlist_project.rename_pin_in_block(block_name, old_name,
                                                     new_name)
            self._log_object_model()
            return True
        except Exception as e:
    ...
    def _sync_net_added(self, block_name: str, net_name: str,
                        pins: list) -> bool:
        if block_name not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Block '{block_name}' not found in model.")
        ...
        try:
            ...
            QMessageBox.warning(self, "Error",
                                f"Failed to add net to model: {e}")
            return False
    ...
    def _sync_net_renamed(self, block_name: str, old_name: str,
                          new_name: str) -> bool:
        if block_name not in self.netlist_project.blocks:
            QMessageBox.warning(self, "Error",
                                f"Block '{block_name}' not found in model.")
            return False
        try:
            self.netlist_project.rename_net_in_block(block_name, old_name,
                                                     new_name)
            self._log_object_model()
            return True
        except Exception as e:
    ...
    def _net_name_exists(self, block_frame: BlockFrame, name: str,
